      ...
      <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 transition-all duration-300">
        <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
          ...
          <div className="flex items-center">
            <Link
              to="/"
              className="text-xl font-bold text-blue-600 hover:text-blue-800 transition-all duration-300"
            >
          ...
          >
            <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
              {['/', '/about', '/contact', '/github', '/skill'].map(
                (path, index) => (
                  <li key={index}>
                    <NavLink
                      to={path}
                      className={({ isActive }) =>
                        `block py-2 pr-4 pl-3 duration-300 border-b border-transparent ${
                          isActive
                            ? 'text-orange-700'
                            : 'text-gray-700'
                        } lg:hover:bg-gray-100 lg:hover:scale-105 lg:transition-all lg:duration-300 lg:p-0`
                      }
                      onClick={closeMenu}
                    >
                      {getLinkName(path)}
